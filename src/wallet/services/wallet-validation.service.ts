...
export class WalletValidationService {
  ...
  async validateAddFunds(userId: number, amount: number): Promise<void> {
    this.logger.log(`🔍 Validando adición de fondos: Usuario ${userId}, Monto: ${amount}`);
    ...
    this.logger.log(`✅ Validación de adición de fondos exitosa: Usuario ${userId}`);
  }

  async validateTransfer(fromUserId: number, toUserId: number, amount: number): Promise<void> {
    this.logger.log(`🔍 Validando transferencia: De ${fromUserId} a ${toUserId}, Monto: ${amount}`);
    ...
    if (!toUser) {
      throw new NotFoundException(`Usuario destinatario con ID ${toUserId} no encontrado`);
    }
    ...
    this.logger.log(`✅ Validación de transferencia exitosa: De ${fromUserId} a ${toUserId}`);
  }

  async validateDeductFunds(userId: number, amount: number): Promise<void> {
    this.logger.log(`🔍 Validando deducción de fondos: Usuario ${userId}, Monto: ${amount}`);
  ...
  async validateOperation(userId: number, operation: string, amount: number, toUserId?: number): Promise<{
    valid: boolean;
    message: string;
    limits: any;
  }> {
    try {
      switch (operation) {
        ...
        case 'transfer':
          if (!toUserId) {
            throw new BadRequestException('toUserId es requerido para transferencias');
          }
      ...
      return {
        ...
        limits: {
          ...
          remainingToday: this.MAX_DAILY_LIMIT - await this.getDailyTransactionTotal(userId),
        },
      };
    }
  }
