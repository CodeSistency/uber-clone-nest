...
export class JwtAuthGuard extends AuthGuard('jwt') {
  canActivate(context: ExecutionContext) {
    ...
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
      console.log(
        '❌ JwtAuthGuard - No se encontró header Authorization o no comienza con Bearer',
      );
    ...
    console.log(
      '🔑 JwtAuthGuard - Token recibido (primeros 20 caracteres):',
  ...
  handleRequest(err: any, user: any, info: any) {
    if (err || !user) {
      console.log(
        '❌ JwtAuthGuard - Error al validar token:',
